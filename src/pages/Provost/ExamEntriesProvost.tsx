...
const ExamEntriesProvost = () => {
  ...
  useEffect(() => {
    if (courseLevel) {
      axios.get('http://localhost:5000/api/getProvostStudents', { params: { course_level: courseLevel } })
        .then(response => {
  ...
  const handleSubmit = () => {
    const entries = students.map(student => ({
      ...
      term: courseLevel,
      provost_status: selectedStudents[student.student_id] ? 'yes' : 'no'
    }));

    axios.post('http://localhost:5000/api/submitEntriesProvost', { entries })
      .then(response => {
        Swal.fire({
          position: 'top-right',
          icon: 'success',
          title: 'You have successfully signed the exam entry form',
...
export default ExamEntriesProvost;
